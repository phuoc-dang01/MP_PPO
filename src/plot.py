...
def bitrate_smo(outputs):
    ...
    SCHEMES = ["bb", "rl", "mpc", "ppo"]
    labels = ["BBA", "Pensieve", "RobustMPC", "Pen-PPO"]
    markers = ["o", "x", "v", "^"]
    lines = ["-", "--", "-.", ":", "-", "--", "-.", ":", "-"]
    reward_all = {}
        ...
        for files in os.listdir(LOG):
            if scheme in files:
                ...
                for line in f:
                    sp = line.split()
                    if len(sp) > 1:
                        bitrate.append(float(sp[1]) / 1000.0)
...
def smo_rebuf(outputs):
    ...
    ax.set_ylim(0.0, max_bitrate * 1.5)
...
if __name__ == "__main__":
    ...
    bitrate_rebuf("tadaa" + "-br")
    smo_rebuf("tada2" + "-sr")
    bitrate_smo("tada3" + "-bs")
